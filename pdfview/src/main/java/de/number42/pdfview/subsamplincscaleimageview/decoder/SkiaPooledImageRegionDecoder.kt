...
class SkiaPooledImageRegionDecoder(bitmapConfig: Bitmap.Config?) : ImageRegionDecoder {
  ...
  override val isReady: Boolean
    @Synchronized get() = decoderPool != null && !decoderPool!!.isEmpty
   ...
   */
  protected fun allowAdditionalDecoder(
    numberOfDecoders: Int,
    fileLength: Long
  ): Boolean {
  ...
  private class DecoderPool {
     ...
     */
    @Synchronized fun add(decoder: BitmapRegionDecoder) {
    ...
    @Synchronized private fun markAsUnused(decoder: BitmapRegionDecoder): Boolean {
      for (entry in decoders) {
